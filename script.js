...
let cart = normalizeCartItems(JSON.parse(localStorage.getItem('elegantlady_cart') || '[]'), []);
let deliveryMethod = localStorage.getItem('elegantlady_delivery_method') || 'pickup';
...
function subscribeToStoreData() {
    ...
    // Phase 1: Load first 6 products fast
    db.collection('products').orderBy('id').limit(6).get().then(function (snapshot) {
        ...
        // Phase 2: Subscribe to all products for real-time updates
        ...
        if (!storeLoadState.products) applyFallbackStoreData('تعذر تحميل المنتجات من فايرستور.');
    });
...
function applySettings() {
    ...
    if (tiktokLink) tiktokLink.href = siteSettings.tiktokLink;
}
...
function saveCart() {
    localStorage.setItem('elegantlady_cart', JSON.stringify(normalizeCartItems(cart, products)));
}

function setDeliveryMethod(method) {
    deliveryMethod = method;
    localStorage.setItem('elegantlady_delivery_method', method);
