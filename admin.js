const ADMIN_USER = 'elegant';
const ADMIN_PASS = '5555';
const FALLBACK_IMAGE = "data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22%3E%3Crect width=%22100%22 height=%22100%22 rx=%2216%22 fill=%22%231a1a1a%22/%3E%3Ctext x=%2250%22 y=%2257%22 text-anchor=%22middle%22 font-size=%2218%22 fill=%22%23c9a96e%22%3EIMG%3C/text%3E%3C/svg%3E";
...
document.addEventListener('DOMContentLoaded', function () {
    if (sessionStorage.getItem('elegantlady_admin') === 'true') {
...
function handleLogin(event) {
    ...
    if (user === ADMIN_USER && pass === ADMIN_PASS) {
        document.getElementById('loginScreen').style.display = 'none';
        document.getElementById('adminPanel').style.display = 'block';
        sessionStorage.setItem('elegantlady_admin', 'true');
        initializeAdmin();
    } else {
...
function logout() {
    sessionStorage.removeItem('elegantlady_admin');
...
function createEmptySize() { return { size: '', unit: 'ml', price: '' }; }
...
function addSizeRow(sizeData) {
    ...
    row.innerHTML = '<input type="number" class="size-value" step="0.1" placeholder="الحجم" value="' + safeSize.size + '"><select class="size-unit"><option value="ml" ' + (safeSize.unit === 'ml' ? 'selected' : '') + '>مل (ml)</option><option value="g" ' + (safeSize.unit === 'g' ? 'selected' : '') + '>غرام (g)</option></select><input type="number" class="size-price" min="0" placeholder="السعر ₪" value="' + safeSize.price + '"><button type="button" class="btn-remove-size" onclick="removeSizeRow(this)">حذف</button>';
...
function renderRevenueChart() {
    ...
    charts.revenue = new Chart(document.getElementById('revenueChart'), {
        type: 'line',
        data: { labels: labels, datasets: [{ label: 'الإيراد', data: values, borderColor: '#c9a96e', backgroundColor: 'rgba(201,169,110,0.15)', fill: true, tension: 0.35 }] },
...
function renderRegionChart() {
    ...
    charts.region = new Chart(document.getElementById('regionChart'), {
        ...
        data: {
            ...
            datasets: [{ data: regions.map(function (region) { return orders.filter(function (order) { return (order.region || 'pickup') === region; }).length; }), backgroundColor: ['#c9a96e', '#d4af37', '#b8860b', '#e8d5a8'] }]
        },
        options: { responsive: true, maintainAspectRatio: false }
    });
}
